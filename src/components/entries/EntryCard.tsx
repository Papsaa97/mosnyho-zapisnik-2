...
export const EntryCard: React.FC<EntryCardProps> = React.memo(({
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 hover:border-slate-700/80 rounded-2xl p-3.5 sm:p-5 shadow-lg transition-all duration-200">
      {/* 1. Calm Top Bar: Status, Work Type & Earnings */}
      <div className="flex items-center justify-between gap-3 mb-2">
        <div className="flex items-center gap-2">
          {/* Status Badge (Koncept / Odevzdáno / Vyfakturováno / Zaplaceno) */}
          ...
          {/* Discreet Work Type Indicator */}
          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[11px] font-bold border ${typeConfig.color}`}>
            <TypeIcon className="w-3 h-3" />
            <span>{typeConfig.label}</span>
          </span>
        </div>

        {/* Grand Total Price & Invoice Number */}
      ...
      {/* 2. Project Name, Client & Date/Time Hierarchy */}
      ...
      {/* 3. Ergonomic Action Bar */}
      <div className="pt-2 border-t border-slate-800 flex items-center justify-between gap-2">
        {/* Toggle Details */}
        <button
          type="button"
          onClick={() => setIsExpanded(!isExpanded)}
          className="text-xs text-slate-400 hover:text-slate-200 flex items-center gap-1 font-semibold py-1 px-1.5 rounded hover:bg-slate-800 transition-colors"
          aria-label={isExpanded ? 'Skrýt detaily' : 'Zobrazit rozpis & poznámku'}
        >
          {isExpanded ? (
            <>
              <ChevronUp className="w-3.5 h-3.5 text-amber-400" />
              <span>Skrýt detaily</span>
            </>
          ) : (
            <>
              <ChevronDown className="w-3.5 h-3.5 text-amber-400" />
              <span>Zobrazit rozpis & poznámku</span>
            </>
          )}
        </button>
      ...
      {/* 4. Structured Expandable Detail (Passports, Cost Breakdown, Photos, Signatures, Notes) */}
      <div className={isExpanded ? 'mt-3.5 pt-3.5 border-t border-slate-800/80 space-y-3 block animate-in fade-in' : 'hidden'}>
        ...
        {/* Travel Breakdown Details if present */}
        ...
        {/* Welding Passport & Badges Protocol Card */}
        {entry.weldingPassport && TechnicalPassportService.shouldRenderPassport(entry.weldingPassport) ? (() => {
          ...
          return (
            <div className="p-3 bg-slate-950/70 rounded-xl border border-amber-500/30 space-y-2.5 text-xs">
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 pb-2">
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  {/* Badges for fast scanning & testability */}
          ...
          entry.weldingMethod && entry.weldingMethod !== 'NONE' && (
            <div className="p-2.5 bg-slate-950/40 rounded-xl border border-slate-800 flex items-center justify-between text-xs">
              <span className="text-slate-400 font-medium">Metoda svařování:</span>
              <span className="px-2 py-0.5 rounded bg-slate-800 text-slate-300 font-mono font-bold border border-slate-700">
                {entry.weldingMethod}
              </span>
            </div>
          )
        ...
        {/* Shift Surcharges & Activity Tags if present */}
        {(((entry.pricing.shiftSurcharges || []).length > 0) || (((entry.activityTags || entry.workActionTags || []).length > 0))) && (
          ...
          <div className="p-2.5 bg-slate-950/40 rounded-xl border border-slate-800">
            ...
            <div className="space-y-1">
              {entry.extraCosts.map((i) => (
                <div key={i.id} className="flex items-center justify-between text-slate-300 text-xs">
          ...
          <div className="p-3 bg-slate-950/70 rounded-xl border border-amber-500/30 space-y-2 text-xs">
            <div className="flex items-center justify-between border-b border-slate-800 pb-1.5">
              <span className="text-[11px] font-bold text-amber-400 uppercase tracking-wider flex items-center gap-1.5">
                ...
                Fotodokumentace ({entry.photos.length} {entry.photos.length === 1 ? 'fotka' : entry.photos.length >= 2 && entry.photos.length <= 4 ? 'fotky' : 'fotek'})
              </span>
        ...
        {/* Technical Notes & Timeline Log */}
        {entry.notes && (
          <div className="bg-slate-950 p-3 rounded-xl border border-slate-800">
            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block mb-1">
              Technická poznámka & protokol:
            </span>
            <p className="font-mono text-slate-300 whitespace-pre-wrap leading-relaxed text-xs">
              {entry.notes}
            </p>
          </div>
        )}

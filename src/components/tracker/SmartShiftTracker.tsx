...
export const SmartShiftTracker: React.FC<SmartShiftTrackerProps> = ({
  timer,
  onFinishShift,
  onOpenManualEntry,
  presets,
  ...
  const {
    ...
    anomalyReason,
    elapsedHours
  } = useShiftElapsed(shiftState, settings?.shiftAnomalyLimitHours || 16);
  ...
  const activePauseFormatted = formatDurationMs(currentPauseDurationMs);
  ...
  // End shift button clicked: check if smart checkout (>16h or overnight) is triggered
  ...
  // User confirmed adjustment in SmartCheckoutModal
  ...
  return (
    ...
    >
      ...
      {/* Warning Banner: Displays when shift > 14 hours or anomaly */}
      {(isWarningLongShift || isAnomaly) && (
        <div className="bg-gradient-to-r from-amber-600/30 via-rose-600/30 to-amber-600/30 border-b border-amber-500/50 px-4 py-2.5 flex items-center gap-2.5 text-amber-300">
          ...
          <div className="text-xs font-bold leading-tight">
            <span className="text-amber-200 font-extrabold block">
              ⚠️ Směna běží podezřele dlouho ({anomalyReason || `${elapsedHours.toFixed(1)} h`})
            </span>
            Nezapomněli jste si odpíchnout? Při ukončení směny vám nabídneme bleskovou opravu skutečného času konce.
          </div>
        </div>
      )}

      {/* Active Pause Notice: Displays when currently paused */}
      {status === 'paused' && (
        <div className="bg-amber-500/10 border-b border-amber-500/30 px-4 py-2 flex items-center justify-between text-xs font-bold text-amber-300">
          <span className="flex items-center gap-1.5">
            <Coffee className="w-4 h-4 text-amber-400 animate-bounce" />
            Přestávka běží (oběd / přejezd):
          </span>
          <span className="font-mono text-sm font-black text-amber-400">
            {activePauseFormatted.display}
          </span>
        </div>
      )}
      ...
      {/* Main Core: Digital Time & Dominant Thumb-Zone Buttons */}
      <div className="p-4 sm:p-6 space-y-4">
          ...
          {status === 'idle' ? (
            <div className="space-y-1">
              ...
              <p className="text-xs sm:text-sm font-semibold text-slate-400">
                Aplikace je připravena k odpíchnutí směny na stavbě či v dílně.
              </p>
            </div>
          ) : (
            <div className="space-y-1.5 w-full">
              ...
              {/* Sub-stats: Start, Total Pause, Net Hours */}
              <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-4 text-xs font-semibold text-slate-400">
                ...
                <span className="flex items-center gap-1 bg-slate-950/80 px-2.5 py-1 rounded-lg border border-slate-800">
                  ...
                  Pauzy: <strong className="text-amber-300 font-mono">{pausedFormatted.display}</strong>
                </span>

                <span className="flex items-center gap-1 bg-slate-950/80 px-2.5 py-1 rounded-lg border border-slate-800">
                  <Clock className="w-3.5 h-3.5 text-sky-400" />
                  Hodin: <strong className="text-sky-300 font-mono">{(netWorkedMs / 3600000).toFixed(1)} h</strong>
                </span>
              </div>
            </div>
          )}
        ...
        {/* Massive Thumb-Zone Action Buttons */}
        <div className="pt-1">
          {status === 'idle' && (
            <div className="space-y-3">
              ...
              <button
                type="button"
                onClick={handleStart}
                className="w-full py-4 sm:py-5 px-6 rounded-2xl bg-gradient-to-r from-emerald-500 via-emerald-400 to-emerald-500 hover:from-emerald-400 hover:to-emerald-300 text-slate-950 font-black text-lg sm:text-xl uppercase tracking-wider flex items-center justify-center gap-3 shadow-xl shadow-emerald-500/25 active:scale-[0.98] transition-all cursor-pointer"
          ...
          {status === 'running' && (
            <div className="grid grid-cols-2 gap-3">
              ...
              <button
                type="button"
                onClick={handlePause}
                className="py-4 px-4 rounded-2xl bg-gradient-to-r from-amber-500 to-amber-400 hover:from-amber-400 hover:to-amber-300 text-slate-950 font-black text-base sm:text-lg uppercase tracking-wider flex items-center justify-center gap-2 shadow-lg shadow-amber-500/20 active:scale-[0.98] transition-all cursor-pointer"
                style={{ minHeight: '64px' }}
              >
                <Pause className="w-6 h-6 fill-slate-950 stroke-[2.5]" />
                <span>Přerušit / Pauza</span>
              </button>
              ...
              <button
                ...
                className="py-4 px-4 rounded-2xl bg-gradient-to-r from-rose-600 to-rose-500 hover:from-rose-500 hover:to-rose-400 text-white font-black text-base sm:text-lg uppercase tracking-wider flex items-center justify-center gap-2 shadow-lg shadow-rose-600/25 active:scale-[0.98] transition-all cursor-pointer"
                style={{ minHeight: '64px' }}
              >
          ...
          {status === 'paused' && (
            <div className="grid grid-cols-2 gap-3">
              ...
              <button
                ...
                className="py-4 px-4 rounded-2xl bg-gradient-to-r from-emerald-500 to-emerald-400 hover:from-emerald-400 hover:to-emerald-300 text-slate-950 font-black text-base sm:text-lg uppercase tracking-wider flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20 active:scale-[0.98] transition-all cursor-pointer"
                style={{ minHeight: '64px' }}
              >
          ...
          <div className="pt-2 border-t border-slate-800 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-black uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
                ...
                Časová osa událostí ({shiftState.events.length})
              </span>
              <button
                type="button"
              ...
              >
                {showTimeline ? 'Skrýt osu' : 'Zobrazit osu'}
              </button>
            </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      placeholder="Zapsat rychlou poznámku z montáže..."
      ...
      {/* Smart Checkout Modal for Forgotten Shift (>16h / overnight) */}
